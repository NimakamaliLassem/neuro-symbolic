...
class TrainerGenerator(object):
    ...
    def zero_state(self, batch_size):
        ...
        h0 = torch.zeros(*state_shape, requires_grad=False, device=self.device)
        if isinstance(self.model.rnn, torch.nn.LSTM):
            c0 = torch.zeros(*state_shape, requires_grad=False, device=self.device)
            return (h0, c0)
        return h0
