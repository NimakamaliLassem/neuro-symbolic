...
from generators_gpt2 import GeneratorGPT2 as Generator
from utils import load_config, save_config, set_logger, set_seed
from trainer_txl import TrainerGenerator, TrainerPredictor
import comm
from viz_utils import generate_and_visualize

class TrainerGeneratorGPT2(TrainerGenerator):
    def zero_state(self, batch_size):
        return None

def parse_args(args=None):
    parser = argparse.ArgumentParser(
        description='RNNLogic GPT-2',
...
def main(args):
    ...
    if comm.get_rank() == 0:
        logging.info('-------------------------')
        logging.info('| Pre-train Generator (GPT-2)')
        logging.info('-------------------------')
    generator = Generator(graph, **cfg.generator.model)
    solver_g = TrainerGeneratorGPT2(generator, gpu=cfg.generator.gpu)
    ...
    for k in range(cfg.EM.num_iters):
        ...
        valid_mrr_iter = solver_p.evaluate('valid', expectation=cfg.predictor.eval.expectation)['mrr']
        test_mrr_iter = solver_p.evaluate('test', expectation=cfg.predictor.eval.expectation)['mrr']
        
        # E-step: Compute H scores of logic rules.
        likelihood = solver_p.compute_H(**cfg.predictor.H_score)
        print("Likelihood sample:", likelihood[:5])
    ...
    if True: 
       try:
            # Generate and save JSON data only. 
            # User can visualize later using visualize_rules.py
            from viz_utils import generate_and_save_data
            generate_and_save_data(solver_g.model, graph, cfg.save_path, num_samples=None) # All relations checking is inside function if None passed
