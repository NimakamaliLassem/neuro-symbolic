...
class TrainerPredictor(object):
    ...
    def train(self, batch_per_epoch, smoothing, print_every):
        ...
        for batch_id, batch in enumerate(islice(dataloader, batch_per_epoch)):
            ...
            if (batch_id + 1) % print_every == 0 or (batch_id + 1) == batch_per_epoch:
                if comm.get_rank() == 0:
                    logging.info('{} {} {:.6f} {:.1f}'.format(batch_id + 1, len(dataloader), total_loss / accum_steps, total_size / accum_steps))
                total_loss = 0.0
                total_size = 0.0
                accum_steps = 0
    ...
    def evaluate(self, split, expectation=True):
        ...
        return {'mrr': mrr, 'mr': mr, 'hit1': hit1, 'hit3': hit3, 'hit10': hit10}
...
class TrainerGenerator(object):
    ...
    def train(self, rule_set, num_epoch=10000, lr=1e-3, print_every=100, batch_size=512):
        if comm.get_rank() == 0:
            logging.info('>>>>> Generator: Training (Mamba)')
        model = self.model
        model.train()
        ...
        for epoch in range(num_epoch):
            ...
            # Mamba doesn't use hidden state
            loss = model.loss(inputs, target, mask, weight, hidden=None)
            loss.backward()

            # Gradient clipping for stability (especially with deeper models)
            ...
            if (epoch + 1) % print_every == 0 or (epoch + 1) == num_epoch:
                if comm.get_rank() == 0:
                    logging.info('{} {} {:.6f}'.format(epoch + 1, num_epoch, total_loss / accum_steps))
                total_loss = 0.0
                accum_steps = 0
    ...
    def log_probability(self, rules):
        ...
        # Mamba doesn't use hidden state
        logits, _ = model(inputs, inputs[:, 0], hidden=None)
    ...
    def next_relation_log_probability(self, seq, temperature):
        ...
        # Mamba processes full sequence, returns logits for all positions
        logits, _ = model(inputs, relation, hidden=None)
    ...
    def beam_search(self, num_samples, max_len, temperature=0.2):
        if comm.get_rank() == 0:
            logging.info('>>>>> Generator: Rule generation with beam search (Mamba)')
        model = self.model
        model.eval()
    ...
    def sample(self, num_samples, max_len, temperature=1.0):
        """
        Sample rules using Mamba. Since Mamba doesn't expose hidden state,
        we re-run the full sequence at each step (works well for short sequences).
        """
        if comm.get_rank() == 0:
            logging.info('>>>>> Generator: Rule generation with sampling (Mamba)')
        model = self.model
        model.eval()
        ...
        for relation in range(model.num_relations):
            ...
            for pst in range(max_len):
                # Run Mamba on full sequence up to current position
                inputs = rules[:, :pst + 1]
                logits, _ = model(inputs, head, hidden=None)
                # Take logits from last position
