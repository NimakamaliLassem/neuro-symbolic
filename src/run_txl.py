...
from generators_txl import GeneratorTransformerXL as Generator
from utils import load_config, save_config, set_logger, set_seed
from trainer_txl import TrainerPredictor, TrainerGenerator
import comm
...
def main(args):
    ...
    if comm.get_rank() == 0:
        logging.info('-------------------------')
        logging.info('| Pre-train Generator (TransformerXL)')
        logging.info('-------------------------')
    ...
    # Visualization Step using the final generator state
    # We use solver_g.model which is the trained generator
    if "Transformer" in cfg.save_path or True: # Always run for this script
       try:
           generate_and_visualize(solver_g.model, graph, cfg.save_path)
